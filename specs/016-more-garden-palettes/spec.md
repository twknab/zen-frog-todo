...
A user opens Options and sees twenty named palettes in a dropdown with tiny color swatches. They pick one; the whole app rethemes immediately. The choice persists across reloads. Fresh visits still default to Natural (Quiet Grove).

**Acceptance Scenarios**:

1. **Given** the app is open, **When** the user opens Options, **Then** they see a Palette dropdown listing all twenty palettes with swatch previews.
...
Light/dark remains orthogonal. Every palette defines both light and dark variants. Text and controls stay readable (WCAG AA) in all forty combinations (20 × 2).

---

### User Story 3 - Options stays calm with many palettes (Priority: P1)

With twenty palettes, Options uses a compact scrollable dropdown (not a crowded button grid). Appearance and Dev stay clear. Changing Palette does not forcibly close Options. On narrow phones the menu scrolls within the viewport.
...
- **FR-001**: System MUST offer exactly twenty named visual palettes per `PALETTE_OPTIONS` (Quiet Grove through Bone Quiet).
...
- **FR-010**: System MUST provide an exclusive Palette **dropdown** listing all twenty options with color swatch previews.
- **FR-011**: The Options Palette control MUST remain calm and uncrowded; it MUST NOT present twenty options as a button grid that crowds the Popover (scrollable menu is required).
...
- **FR-015**: All forty palette×appearance combinations MUST meet WCAG AA contrast for text and interactive elements.
...
- **FR-018**: Body typography MUST remain Manrope. High-energy palettes (Prism Bloom, Web Ring, Borealis, Mirrorball, Sugar Rush, Starfruit, Firefly, Tropic Punch, Emberglow) MAY use the playful display heading face; Quiet Grove, Violet Hour, Golden Hour, Tideglass, Frostbloom, Sakura Drift, and all five high-contrast greyscale palettes use the calm Zen heading face.
- **FR-019**: Brand/wordmark MAY use a joyful gradient for colorful opt-in palettes; Quiet Grove, Violet Hour, Ash Terrace, and Bone Quiet MUST keep solid brand coloring. Graphite Grove, Ink Well, and Obsidian Pond MAY use a subtle moss→steel gradient (premium, not carnival).
...
- **Palette preference**: persisted id among the twenty `PaletteId`s; default `natural`.
...
- **SC-003**: All twenty palettes are usable in both light and dark with readable primary text and controls.
