...
- Q: High-contrast / greyscale themes? → A: Five more palettes — **Graphite Grove**, **Ink Well**, **Ash Terrace**, **Obsidian Pond**, **Bone Quiet** — grey/scale/black energy with stronger AA contrast. Dark-mode-first vibe OK; each still defines light + dark. Personality in surfaces/ink/mist; fruit accents muted greys/silvers/sparse cool accents; moss stays green-family. Calm Zen heading; solid or subtle (not carnival) wordmarks.
- Q: True-black / OLED theme? → A: One more palette — **Blackout** (`blackout`) — real `#000` dark surfaces (not charcoal), stark high-contrast light paper, green-family moss, sorts last in Options. Distinct from Ink Well / Obsidian Pond / Graphite Grove.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Discover and pick a joyful palette (Priority: P1)

A user opens Options and sees twenty-one named palettes in a dropdown with tiny color swatches. They pick one; the whole app rethemes immediately. The choice persists across reloads. Fresh visits still default to Natural (Quiet Grove).

**Acceptance Scenarios**:

1. **Given** the app is open, **When** the user opens Options, **Then** they see a Palette dropdown listing all twenty-one palettes with swatch previews.
...
Light/dark remains orthogonal. Every palette defines both light and dark variants. Text and controls stay readable (WCAG AA) in all forty-two combinations (21 × 2).

---

### User Story 3 - Options stays calm with many palettes (Priority: P1)

With twenty-one palettes, Options uses a compact scrollable dropdown (not a crowded button grid). Appearance and Dev stay clear. Changing Palette does not forcibly close Options. On narrow phones the menu scrolls within the viewport.
...
- **FR-001**: System MUST offer exactly twenty-one named visual palettes per `PALETTE_OPTIONS` (Quiet Grove through Blackout).
...
- **FR-010**: System MUST provide an exclusive Palette **dropdown** listing all twenty-one options with color swatch previews.
- **FR-011**: The Options Palette control MUST remain calm and uncrowded; it MUST NOT present twenty-one options as a button grid that crowds the Popover (scrollable menu is required).
...
- **FR-015**: All forty-two palette×appearance combinations MUST meet WCAG AA contrast for text and interactive elements.
...
- **FR-018**: Body typography MUST remain Manrope. High-energy palettes (Prism Bloom, Web Ring, Borealis, Mirrorball, Sugar Rush, Starfruit, Firefly, Tropic Punch, Emberglow) MAY use the playful display heading face; Quiet Grove, Violet Hour, Golden Hour, Tideglass, Frostbloom, Sakura Drift, and all six high-contrast greyscale/black palettes use the calm Zen heading face.
- **FR-019**: Brand/wordmark MAY use a joyful gradient for colorful opt-in palettes; Quiet Grove, Violet Hour, Ash Terrace, and Bone Quiet MUST keep solid brand coloring. Graphite Grove, Ink Well, Obsidian Pond, and Blackout MAY use a subtle moss→steel gradient (premium, not carnival).
- **FR-020**: Primary/`moss` MUST stay green-family across all palettes so ground frogs and canopy leaves remain a living green pile; canopy frog-fruit MUST draw from non-green accents (greyscale themes MAY mute fruit to greys/silvers/sparse cool accents).
...
- **FR-024**: Graphite Grove MUST express cool charcoal/slate greys with crisp contrast.
- **FR-025**: Ink Well MUST express noir near-black energy with sharp white/ink text (high contrast).
- **FR-026**: Ash Terrace MUST express warm concrete greys (not pure cold slate).
- **FR-027**: Obsidian Pond MUST express deep void black with a sparse cool silver/teal accent while moss stays green-family.
- **FR-028**: Bone Quiet MUST express chalk/bone light-leaning true greyscale with strong AA contrast.
- **FR-029**: Blackout MUST express OLED true black (`#000` dark surfaces) as the blackest, highest-contrast basically-black option — distinct from Ink Well, Obsidian Pond, and Graphite Grove — with stark high-contrast light paper and green-family moss. It MUST sort last in `PALETTE_OPTIONS` / `PALETTE_IDS`.

### Key Entities

- **Palette preference**: persisted id among the twenty-one `PaletteId`s; default `natural`.
...
- **SC-003**: All twenty-one palettes are usable in both light and dark with readable primary text and controls.
