...
- [ ] T002 [US1] In `src/app/page.tsx`, add an info `IconButton` (e.g. `InfoOutlined` from `@mui/icons-material`) wrapped in MUI `Tooltip` to the Bonsai section's header `Stack` (~lines 320-325, next to the existing `SpaOutlinedIcon` + "Bonsai" `Typography`), mirroring the exact `Tooltip`+`IconButton` pattern already used at `page.tsx:162`. The tooltip's `title` is "Grows as you finish tasks and focus sessions." The `IconButton` has an explicit `aria-label` (e.g. "About the bonsai") so its purpose is announced independent of the tooltip.
