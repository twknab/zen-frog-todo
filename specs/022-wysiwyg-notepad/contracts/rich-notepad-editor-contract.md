...
5. **Formatting UX (revised 2026-08-09)**: a compact formatting toolbar using classic word-processor iconography — bold, italic, strikethrough, H1/H2, bullet list, numbered list, task list, link, quote, inline code — plus markdown input rules (type markdown → live formatting). Toolbar placement: **desktop (md+)** always-visible sticky top bar; **phones** a selection bubble (TipTap `BubbleMenu`) that appears when text is selected — matching common mobile editing apps and avoiding iOS focus-zoom + keyboard collision with a docked bottom bar. Editor text MUST be ≥16px on phones so Safari does not auto-zoom on focus. Horizontally scrollable, never wrapping; ≥44px touch targets. Active formats show a soft selected state. All controls `aria-label`led, keyboard-reachable, `aria-pressed` where applicable.
