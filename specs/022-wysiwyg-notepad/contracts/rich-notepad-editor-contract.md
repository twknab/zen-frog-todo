...
5. **Formatting UX (revised 2026-08-09)**: a compact formatting toolbar using classic word-processor iconography — bold, italic, strikethrough, H1/H2, bullet list, numbered list, task list, link, quote, inline code — plus markdown input rules (type markdown → live formatting). Toolbar placement: **desktop (md+)** always-visible sticky top bar; **phones** a fixed keyboard-docked format strip (ported to `document.body`, pinned with `visualViewport`) that appears while the editor is focused or has a text selection — TipTap `BubbleMenu` is unreliable with iOS touch selection. Editor text MUST be ≥16px on phones so Safari does not auto-zoom on focus. Horizontally scrollable, never wrapping; ≥44px touch targets. Active formats show a soft selected state. All controls `aria-label`led, keyboard-reachable, `aria-pressed` where applicable.
