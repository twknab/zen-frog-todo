...
- Mode remains parent-controlled and shared across tabs (`mode`/`onModeChange`), default `"write"` on open — unchanged mechanism.
...
5. **Formatting UX**: markdown input rules active (type markdown → live formatting) + a minimal selection bubble menu (bold/italic/link/heading). No heavy persistent toolbar. All controls `aria-label`led and keyboard-reachable.
