...
- **Classic-toolbar formatting UX (revised 2026-08-01 per user direction)**: a **compact persistent toolbar** using the universally recognized word-processor pattern — icon buttons for bold/italic/strike, H1/H2, bullet/numbered/task lists, link, quote, inline code — themed calm (ghost icon buttons, muted colors, active state = soft primary fill). Markdown **input rules** stay active as the power-user path (`**bold**`, `# `, `- [ ] ` all format live). **Mobile-first**: toolbar docks to the bottom of the editing surface on phone widths (thumb + above-keyboard reachable), scrolls horizontally instead of wrapping, ≥44px touch targets; on desktop it sits at the top of the editor. Recognition over novelty (FR-013/FR-015).
